...
|---|----------|-----------|------------|
| 1 | Создание схемы | ✅ Успешно | Схема `pm` создана |
| 2 | Создание таблиц | ✅ Успешно | 4 таблицы: employees, projects, tasks, assignments |
| 3 | Вставка данных | ✅ Успешно | 24 записи (5+3+8+8) |
| 4 | SELECT запросы | ✅ Успешно | Все запросы отработали корректно |
| 5 | JOIN запросы | ✅ Успешно | Связи между таблицами работают |
| 6 | Статистика | ✅ Успешно | Данные соответствуют ожидаемым |
...
| Приоритет | Количество | Процент | Важность |
|-----------|------------|---------|----------|
| Высокий | 5 | 62.5% | 🔴 критичные |
| Средний | 3 | 37.5% | 🟡 важные |
| Низкий | 0 | 0% | 🟢 не срочные |
...
|---|------------|----------|-------------|
| 1 | Приоритеты задач | 62.5% | 5 из 8 задач имеют высокий приоритет |
| 2 | Загрузка команды | Иван Петров | Самый загруженный сотрудник (3 задачи) |
| 3 | Распределение проектов | 2-3 задачи | Все проекты имеют равномерную нагрузку |
| 4 | Статус проектов | 2 в работе, 1 планируется | 67% проектов активны |
| 5 | Роли сотрудников | 5 ролей | Менеджер, 2 разработчика, аналитик, тестировщик |
...
|---|--------------|-------------|
| 1 | Пересмотреть приоритеты задач | Высокий приоритет > 60% — это много |
| 2 | Перераспределить задачи | Иван Петров перегружен (3 задачи) |
| 3 | Ускорить запуск проекта | Проект "Внедрение BI" находится в статусе "Планируется" |
